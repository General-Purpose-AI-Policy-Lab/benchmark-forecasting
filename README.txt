...
\[
y_{i,t} \sim \operatorname{SkewNormal}\big(\mu_i(t),\, \sigma_i(t),\, \alpha^{\text{skew}}_i\big),
\]
...
- **Asymptotes:**
  ...
  - Task-level \(L_i\) are obtained via shifted Beta draws:
    \[
    L^{\text{raw}}_i \sim \operatorname{Beta}(L_{\mu}, L_{\sigma}), \quad
...
- **Growth rates (logistic and Harvey):**
  ...
  - Task-level parameters:
    \[
    k_i \sim \operatorname{Gamma}(k_{\mu}, k_{\sigma}) \quad \text{or} \quad
    r_i \sim \operatorname{Gamma}(r_{\mu}, r_{\sigma}).
    \]
...
- **Noise scales and skewness:**
  - Shared hyperpriors \(\sigma^{\text{base}}_{\mu}, \sigma^{\text{base}}_{\sigma}\) and \(\alpha^{\text{skew}}_{\mu}, \alpha^{\text{skew}}_{\sigma}\).
  - Task-level \(\sigma^{\text{base}}_i\) and \(\alpha^{\text{skew}}_i\) are drawn from these distributions.

- **Harvey shape parameter:**
  - Hyperpriors \(\alpha^{\text{base}}_{\mu}, \alpha^{\text{base}}_{\sigma}\) for a base parameter,
